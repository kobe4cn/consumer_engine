...
Infer the repo from `git remote -v` — `gh` does this automatically when run inside a clone. Repo: `kobe4cn/consumer_engine` (branch `main`).
...
- **Blocking**: GitHub's native issue dependencies. Where unavailable, fall back to a `Blocked by: #<n>` line at the top of the child body. A ticket is unblocked when every blocker is closed.
